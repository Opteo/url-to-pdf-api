...
async function render(_opts = {}) {
  ...
  const browser = await puppeteer.launch({
    ...
    args: ['--disable-gpu', '--no-sandbox', '--disable-setuid-sandbox', '--font-render-hinting=none'],
  ...
  try {
    ...
    if (this.failedResponses.length) {
      ...
      if (opts.failEarly === 'all') {
        const err = new Error(
          `${
            this.failedResponses.length
          } requests have failed. See server log for more details.`
        )
        err.status = 412
        throw err
      }
